 ...
 */
 ...
 function getMousePosition(e)
  {
   ...
   if (pMap_IE)
    {
		var theElement = document.getElementById(pMap_ImageID);
		  var selectedPosX = 2;
		  var selectedPosY = 2;
		...
		var imgX = selectedPosX -= document.documentElement.scrollLeft;
		var imgY = selectedPosY -= document.documentElement.scrollTop;
		  
		//pMap_MouseX = event.clientX + document.documentElement.scrollLeft; 
		//pMap_MouseY = event.clientY + document.documentElement.scrollTop;

		pMap_MouseX = event.clientX - imgX; 
		pMap_MouseY = event.clientY - imgY;
		
		//alert("mouse: " + event.clientX + "," + event.clientY);
		//alert("scroll: " + document.documentElement.scrollLeft + "," + document.documentElement.scrollTop);
		//alert("offset: " + document.getElementById(pMap_ImageID).offsetLeft + "," + document.getElementById(pMap_ImageID).offsetTop);
   //pMap_MouseX -= document.getElementById(pMap_ImageID).offsetLeft - document.body.scrollLeft;
   //pMap_MouseY -= document.getElementById(pMap_ImageID).offsetTop - document.body.scrollTop;
	}
   else
    { 
		pMap_MouseX = e.pageX; 
   ...
   for (Map in pMap_ImageMap)
    {
	 ...
	 //alert("checking map: " + Values[0] + "," + Values[1] + "," + Values[2] + "," + Values[3] + " against " + pMap_MouseX + "," + pMap_MouseY);
 ...
 function LoadImageMap(ID,iURL)  { 
	pMap_ImageID = ID, pMap_URL = iURL; 
	AjaxLoad();
	//addLoadEvent(AjaxLoad);
}